...
def predict_mis(model, adj_list, device, time_budget=120, num_maps=32, max_solutions=16):
    ...
    while time.time() - start_time < time_budget and not Q.empty():
        ...
        for m in range(num_maps):
            ...
            # keep top 25% vertices (adaptive threshold)
            k = max(1, int(1 * len(probs)))

            top_indices = torch.topk(probs, k).indices

            # sort selected vertices by score (descending)
            sorted_indices = top_indices[torch.argsort(probs[top_indices], descending=True)]
            ...
            for v in vertices:
                if v in new_state.labels:  # If we hit a labeled vertex, we continue with next map
                    continue
...
def find_colors(adj_list, model_path, device):
    start_time = time.time()
    hidden_dim = 32
    num_layers = 20
...
def predict_value_aware_mis(model,adj_list,select_values,nonselect_values,device,time_budget=120,num_maps=32,max_solutions=16,max_size=4):
    ...
    def update_queue(Q, new_state, max_size):
        ...
        # Sort states by number of unlabeled nodes (lower is better)
        states.sort(key=lambda x: x.get_unlabeled_nodes())

        # Keep only the first max_size states ()
        states = states[:max_size]
    ...
    while not Q.empty() and time.time() - start_time < time_budget:
        ...
        for m in range(min(num_maps, prob_maps.size(1))):
            ...
            # convert select_values to tensor if it is a list
            select_tensor = torch.tensor(sub_select, device=probs.device, dtype=probs.dtype)

            # compute combined score (probability × select_value)
            scores = probs * select_tensor
            ...
            if new_state.is_completely_labeled():
                ...
                if sig not in seen_signatures:
                      Q.put(new_state)
                      update_queue(Q, new_state, max_size)
...
def predict_value_colors(model, adj_list, device, time_budget=300, max_queue_size=4):
    ...
    def update_queue(Q, new_state, max_size):
        ...
        states.sort(
            key=lambda x: x.get_edge_density()
        )
    ...
    while not Q.empty() and time.time() - start_time < time_budget:
        ...
        for reduced_mis in model_mis_list:
            ...
            for sub_idx, decision in reduced_mis.items():
                ...
                elif decision == 1 and new_state.values[red_node]["nonselect_value"] > 0:
...
def color_graph(adj_list, model_path, device):
...
def load_cora_as_networkx():
    """
    Load the Cora dataset and convert it to a NetworkX graph.
...
def load_citeseer_as_networkx(root="./data"):
...
def load_pubmed_as_networkx(root="./data"):
...
def load_ego_facebook_as_networkx():
  ...
  # Load into NetworkX
  G_fb = nx.read_edgelist(txt_path,delimiter=" ",nodetype=int)
  return G_fb
...
if __name__ == "__main__":
    ...
    G = load_cora_as_networkx() # choose graph accordingly 
    reducer = Degree12Reducer(G)
    reduced_G, stats,checker = reducer.reduce_graph(d) # reduces the graph to d%
    print("Original nodes:", G.number_of_nodes())
    print("Reduced nodes:", reduced_G.number_of_nodes())
    print("Stats:", stats)
    adj_list=networkx_to_adj_list(G)
    find_colors(adj_list, model_path1, device)
    if G.number_of_nodes() == reduced_G.number_of_nodes():
        print("No reduction")
    else:
        color_graph(adj_list, model_path2, device)
